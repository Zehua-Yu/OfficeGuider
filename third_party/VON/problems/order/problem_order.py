...
class Order(object):
    ...
    def get_costs(dataset, pi, metric='tsp'):
        ...
        # 与输入同设备，便于 CPU 推理（SmartFlow 前端调用）
    ...
    def __init__(self, filename=None, size=50, num_samples=32, offset=0, distribution='None', mode='train', mission='FM', dataset_number=1, epoch=0):
        ...
        if mission == 'FM':
            ...
            for i in range(1):
                data_tsne[i, :, :] = data
            self.data = [data_tsne, images]
        elif mission == 'smartflow':
            with open(f'data/ordering/long_10k_xy_tensors/all_xy_L50_shuffled_input.pkl', 'rb') as f1:
                data_tsne = pickle.load(f1)
            self.data = data_tsne
        elif mission == 'user_pkl':
            if filename is None:
                raise ValueError("mission user_pkl 需要 --train_dataset 指向 pickle 文件")
            ...
            rows = []
            for row in raw:
                t = torch.as_tensor(row, dtype=torch.float32)
                if t.dim() != 2 or t.size(-1) != 2:
                    raise ValueError("user_pkl: 每个实例须为 [N,2] 浮点数组")
                rows.append(t)
            if not rows:
                raise ValueError("user_pkl: 数据集为空")
            need = offset + num_samples
            cycled = []
            for i in range(need):
                cycled.append(rows[(offset + i) % len(rows)])
            self.data = cycled
        else: #TSP
