...
```commandline
python run.py --sample_size 50 --run_name test --mission FM --metric tsp
```
...
**Note (OfficeGuider in this repo):** The `--mission` flag only selects which **builtin evaluation dataset** is constructed in `eval.py` (`model.problem.make_dataset(..., mission=...)`). It does **not** affect weight loading: both `eval.py` and OfficeGuider use the same `load_model(model_dir)` path (`args.json` + `epoch-*.pt`). OfficeGuider does not run `eval.py`; it loads the checkpoint the same way, then feeds **your** 2D coordinates (from `data/ordering/atom_embeddings_2d/atoms_2d.json`) into `sample_many` with greedy decoding—the same core call as the greedy branch in `eval.py`. You do not pass `mission` to the OfficeGuider HTTP API; set `SMARTFLOW_VON_PRETRAINED` to the same directory you would pass as `--model`.
